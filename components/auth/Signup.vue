        ...
        <div class="w-full h-full flex justify-center">
            <div class="w-[40%] h-full bg-slate-700 p-5 flex flex-col justify-center items-center gap-4">
                <div class="flex justify-center items-center gap-4">
                    ...
                    <p class="text-slate-100 font-semibold text-4xl">Interview<span
                            class="text-transparent bg-clip-text bg-gradient-to-r from-pink-300 to-blue-300">Sense</span>
                    </p>
                </div>
            ...
            <div class="w-[60%] flex flex-col items-center justify-center gap-4">
                ...
                <div
                    class="transition-all duration-300 p-[2px] rounded-lg bg-gradient-to-br from-blue-300 to-pink-300 w-[70%]">
                    <input type="password" class="w-full p-5 rounded-md outline-none text-lg"
                        placeholder="Retype Password: " v-model="retype" />
                </div>
                ...
                <div class="relative inline-flex items-center justify-center w-full">
                    <hr class="w-64 h-px my-8 bg-gray-200 border-[1px] border-slate-300 dark:bg-slate-600">
                    <span class="absolute px-3 font-normal text-slate-600 bg-white mx-auto dark:text-slate-400 "
                        @click="() => { login = true; signup = false }">Sign In Here :)</span>
                </div>
            <div :class="`group relative flex flex-row items-center transition ease-in-out duration-300 ${disabled ? '' : 'hover:-translate-y-1 hover:opacity-[0.85]'} w-48 h-24`"
                ...
                <button :disabled="disabled"
                    ...
                    v-if="!disabled"></button>
                <div
                    :class="` ${disabled ? 'bg-slate-300 cursor-default' : 'group-hover:shadow-lg bg-slate-800 cursor-pointer'} text-white duration-300 transition ease-in-out text-2xl font-light rounded-lg h-14 w-40 text-center flex justify-center items-center z-20 absolute left-4 top-5`">
                    Sign Up</div>
            </div>
